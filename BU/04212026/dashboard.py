        ...
        if not st.session_state.tab2_filtro_conc:
            ...
            if not lista_artigos_final:
                ...
                if df_hist.empty:
                    ...
                    # ============================================================
                    # RESUMO POR CONCORRENTE (COM CORES VERDE/VERMELHO)
                    # ============================================================
                    with st.expander("📊 Resumo por Concorrente", expanded=False):
                        ...
                        stats = pd.DataFrame(resumo_data).sort_values("Média")
...
with tab4:
    ...
    # ============================================================
    with st.expander("📋 Concorrentes Ativos (configuração)", expanded=False):
        if concorrentes_ativos:
            conc_pt = [c for c in concorrentes_ativos if c.startswith("PT_")]
            conc_es = [c for c in concorrentes_ativos if c.startswith("ES_")]

            col_pt, col_es = st.columns(2)
            with col_pt:
                st.markdown("**Portugal**")
                if conc_pt:
                    for c in sorted(conc_pt):
                        nome = formatar_nome_concorrente(c)
                        url = concorrentes_ativos[c]
    ...
    st.divider()
    st.caption("💡 A lista de concorrentes ativos é guardada em 'concorrentes_ativos.json'. O script de atualização do catálogo deve usar esse ficheiro para saber quais folhas criar/atualizar.")

# ============================================================
# TAB 5 – GESTÃO DE LINKS (COM BACKUP E MAIOR ROBUSTEZ)
# ============================================================
with tab5:
    ...
    def criar_backup():
        """Cria uma cópia de segurança do Excel com timestamp."""
    ...
    if df_dm_gestao.empty:
        ...
        st.error("⚠️ Nenhum concorrente encontrado no ficheiro Excel. O ficheiro pode estar corrompido ou sem folhas de concorrentes.")
        if st.button("🔄 Tentar recarregar"):
            st.cache_data.clear()
            st.rerun()
        st.info("💡 Se o problema persistir, restaure uma cópia de segurança da pasta 'backups' ou do ficheiro original.")
    else:
        # Dicionário de informações do Douromed
        dict_info_dm = {}
        ...
        def obter_ultimo_preco(artigo: str, concorrente: str) -> tuple:
            try:
                ...
                if result and result[0] and float(result[0]) > 0:
                    ...
                    return (f"{preco_val:.2f} € ({data_raw})", preco_val, data_raw)
        ...
        def carregar_workbook(conc: str):
            ...
            try:
                ...
                if conc not in wb.sheetnames:
                    return None, f"Folha '{conc}' não encontrada no ficheiro."
                ws = wb[conc]
                mapa = {}
                for row in ws.iter_rows(min_row=2, max_col=1):
                    if row[0].value:
                        artigo = str(row[0].value).strip()
                        mapa[artigo] = row[0].row
        ...
        def salvar_e_recarregar(conc: str, wb):
            try:
                ...
                if not sucesso_backup:
                    st.warning(f"⚠️ Não foi possível criar backup: {msg_backup}")
                ...
                return False, "Permissão negada. Feche o Excel se estiver aberto."
            except Exception as e:
        ...
        if not st.session_state.conc_sel_gestao:
            ...
            if not arts_filtrados:
                ...
                url_atual = ""
                ws = wb[conc_atual]
                linha_artigo = mapa_linhas.get(artigo_sel)
                if linha_artigo:
                    val = ws.cell(row=linha_artigo, column=3).value
                    url_atual = str(val) if val and str(val).lower() not in ("", "nan", "none") else ""

                info_dm = dict_info_dm.get(artigo_sel, {})
                ultimo_preco, _, _ = obter_ultimo_preco(artigo_sel, conc_atual)
                    ...
                    try:
                        ...
                        if col_url:
                            ...
                            if artigos_sem_link:
                                ...
                                st.download_button("📥 Exportar Excel", output.getvalue(), file_name=f"artigos_sem_link_{conc}.xlsx")
                            else:
