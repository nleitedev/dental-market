...
with tab5:
    ...
    def get_concorrentes_excel():
        try:
            # Verificar se o ficheiro existe e é legível
            if not os.path.exists(EXCEL_PATH):
                return []
    ...
    if df_dm_gestao.empty:
        ...
        if 'workbook_cache' not in st.session_state:
            st.session_state.workbook_cache = {}  # {conc: (workbook, mapeamento_linhas)}

        def carregar_workbook(conc: str):
            """Carrega workbook e mapeamento (cache opcional)."""
            # Sempre carregar do disco para evitar inconsistências após múltiplos salvamentos
            # Mas podemos usar cache se quisermos performance; vamos usar cache com cuidado.
        ...
        def salvar_e_recarregar(conc: str, wb):
            """Salva o workbook no disco, limpa cache e recarrega."""
            try:
                ...
                wb.save(EXCEL_PATH)
                # Limpar cache deste concorrente para forçar recarga fresca
                if conc in st.session_state.workbook_cache:
                    del st.session_state.workbook_cache[conc]
                # Limpar caches de dados derivados
                st.cache_data.clear()
        ...
        if not st.session_state.conc_sel_gestao:
            ...
            if not arts_filtrados:
                ...
                # Obter URL atual usando mapeamento
                url_atual = ""
                ws = wb[conc_atual]
    ...
    # Lista de artigos sem link (com cache)
