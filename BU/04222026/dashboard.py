...
def query_comparacao(concorrentes_filtro: tuple, pesquisa: str, marcas: tuple, familias: tuple):
    ...
    df = pd.read_sql(f"""
        WITH ranked AS (
            SELECT artigo, descricao, concorrente, CAST(preco AS REAL) as preco, url, data, referencia,
        ...
        SELECT artigo, descricao, concorrente, preco, url, data, referencia FROM ranked WHERE rn=1
...
def query_historico(artigo: str, concorrentes_filtro: tuple, dias: int):
    ...
    df = pd.read_sql(f"""
        SELECT data, concorrente, CAST(preco AS REAL) as preco, promo, referencia
...
def query_alertas():
    ...
    return pd.read_sql("""
        WITH ranked AS (
            SELECT artigo, descricao, concorrente, CAST(preco AS REAL) as preco, data, referencia,
                   ROW_NUMBER() OVER (PARTITION BY artigo, concorrente ORDER BY data DESC) as rn
            FROM precos WHERE sucesso=1
        ),
...
def eliminar_concorrente(concorrente):
    """
    Elimina um concorrente da base de dados SQLite e do ficheiro Excel.
    Retorna: (sucesso, registos_afetados, excel_removido, mensagem_erro)
    """
    try:
        # 1. Eliminar da base de dados
        conn = obter_conn()
        cursor = conn.execute("DELETE FROM precos WHERE concorrente = ?", (concorrente,))
        registos = cursor.rowcount
        conn.commit()
        # NOTA: NÃO fechar a conexão – é gerida pelo cache do Streamlit.
        
        # 2. Eliminar a folha do Excel
        excel_ok = False
        erro_excel = ""
        if os.path.exists(EXCEL_PATH):
            try:
                ...
                if concorrente in wb.sheetnames:
                    ...
                    erro_excel = "Folha não encontrada no Excel."
            except PermissionError:
                erro_excel = "Permissão negada. Feche o Excel se estiver aberto."
            except Exception as e:
                erro_excel = str(e)
        
        if not excel_ok and erro_excel:
            return False, registos, False, erro_excel
    ...
    if not conc_sel:
        ...
        if df_raw.empty:
            ...
            if st.button("📥 Exportar para Excel"):
                ...
                # 1. Dicionários de preços numéricos e referências da BD
                preco_map = {}
                ref_bd_map = {}
                if not df_raw.empty:
                ...
                # 2. Ler referências manuais do Excel
                ref_manual_map = {}
                try:
                    wb_excel = openpyxl.load_workbook(EXCEL_PATH, read_only=True)
                    for sheet_name in wb_excel.sheetnames:
                        if sheet_name.lower() == "douromed":
                            continue
                        ws = wb_excel[sheet_name]
                        for row in ws.iter_rows(min_row=2, max_col=4, values_only=True):
                            if row[0] is None:
                                continue
                            artigo = str(row[0]).strip()
                            url = row[2] if len(row) > 2 else ""
                            ref_manual = row[3] if len(row) > 3 else ""
                            # Só consideramos se existir URL (artigo monitorizado)
                            if url and str(url).startswith("http"):
                                key = (artigo, sheet_name)
                                if ref_manual and str(ref_manual).strip().lower() not in ("", "nan", "none"):
                                    ref_manual_map[key] = str(ref_manual).strip()
                    wb_excel.close()
                except Exception as e:
                    st.warning(f"Não foi possível ler referências manuais: {e}")

                # 3. DataFrame base com colunas fixas
                ...
                for conc_original in conc_sel:
                    ...
                    if col_preco_fmt in pivot_preco.columns:
                        ...
                        df_export[col_preco_fmt] = df_export.apply(
                            lambda row, c=conc_original: preco_map.get((str(row["Artigo"]), c), None),
                            axis=1
                        )
                        colunas_preco.append(col_preco_fmt)

                        # Referência combinada
                        def obter_ref_combinada(row, c=conc_original):
                            key = (str(row["Artigo"]), c)
                            manual = ref_manual_map.get(key, "")
                            if manual:
                                return manual
                            return ref_bd_map.get(key, "")
                        df_export[col_ref] = df_export.apply(obter_ref_combinada, axis=1)
                        colunas_ref.append(col_ref)

                # 4. Substituir NaN/None por vazio
                df_export = df_export.fillna("")

                # 5. Escrever para Excel
                wb = Workbook()
                ws = wb.active
                ws.title = "Comparação"
                ...
                # 6. Formato texto para colunas de referência
...
with tab2:
    if not todos_art:
        ...
        # ============================================================
        # CRIAR DICIONÁRIO DE INFORMAÇÕES DO DOUROMED
        # ============================================================
        dict_info_dm_tab2 = {}
        if not df_dm.empty:
        ...
        def ordem_pais(nome):
            if nome.startswith("PT_"):
                return (0, nome)
            elif nome.startswith("ES_"):
                return (1, nome)
            else:
                return (2, nome)
        ...
        with st.expander("🔎 Filtros", expanded=True):
            ...
            with col_pais:
                ...
                if (inc_pt != st.session_state.tab2_inc_pt or 
                    inc_es != st.session_state.tab2_inc_es or 
                    inc_out != st.session_state.tab2_inc_out):
                    ...
                    if inc_pt:
                        nova_selecao.update(conc_pt)
                    else:
                        nova_selecao.difference_update(conc_pt)
                    if inc_es:
                        nova_selecao.update(conc_es)
                    else:
                        nova_selecao.difference_update(conc_es)
                    if inc_out:
                        nova_selecao.update(conc_outros)
                    else:
                        nova_selecao.difference_update(conc_outros)
            ...
            with col_conc:
                opcoes_conc = []
                if inc_pt:
                    opcoes_conc.extend(conc_pt)
                if inc_es:
                    opcoes_conc.extend(conc_es)
                if inc_out:
                    opcoes_conc.extend(conc_outros)
                ...
                conc_sel = st.multiselect(
                    "🏢 Concorrentes:",
                    options=opcoes_conc,
                    default=st.session_state.tab2_filtro_conc,
        ...
        if not st.session_state.tab2_filtro_conc:
            ...
            if not lista_artigos_final:
                ...
                with st.spinner("A carregar histórico..."):
                    df_hist = query_historico(
                        artigo_sel,
                        tuple(st.session_state.tab2_filtro_conc),
                        dias_map[periodo]
                    )
                ...
                if df_hist.empty:
                    ...
                    fig = px.line(
                        df_hist, x="data", y="preco", color="concorrente_fmt", markers=True,
                    ...
                    if pvp_dm and not pd.isna(pvp_dm):
                        fig.add_hline(
                            y=pvp_dm, line_dash="dash", line_color="green",
                    ...
                    # ============================================================
                    # TABELA DE HISTÓRICO (COM ORDENAÇÃO)
                    # ============================================================
                    ...
                    # ============================================================
                    # RESUMO POR CONCORRENTE (COM ORDENAÇÃO ALFABÉTICA)
                    # ============================================================
                    with st.expander("📊 Resumo por Concorrente", expanded=False):
                        ...
                        # Ordenar por nome formatado (alfabético)
...
# ============================================================
# TAB 4 – GESTÃO DE CONCORRENTES (COM JSON DE ATIVOS)
# ============================================================
with tab4:
    import json
    from pathlib import Path
    ...
    CONFIG_FILE = Path(EXCEL_PATH).parent / "concorrentes_ativos.json"

    def carregar_concorrentes_ativos():
        """Carrega o dicionário {concorrente: homepage} do ficheiro JSON."""
        if CONFIG_FILE.exists():
            ...
            # Valores padrão iniciais (os mesmos que já existem)
            default = {
            ...
            guardar_concorrentes_ativos(default)
            return default

    def guardar_concorrentes_ativos(ativos_dict):
        """Guarda o dicionário no ficheiro JSON."""
        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
            json.dump(ativos_dict, f, indent=4, ensure_ascii=False)

    # Carregar concorrentes ativos
    concorrentes_ativos = carregar_concorrentes_ativos()

    # Obter também as folhas reais do Excel para mostrar estado atual
    ...
    # ============================================================
    # SEÇÃO 1: ATUALIZAR CATÁLOGO (agora respeita o JSON)
    # ============================================================
    with st.expander("🔄 Actualizar Catálogo (Douromed + Concorrentes)", expanded=False):
        ...
        - 📝 Actualizar **apenas** as folhas dos concorrentes listados no ficheiro de configuração.
        """)
                    ...
                    if os.path.exists(script_path):
                        ...
                        if result.returncode == 0:
                            ...
                            if novos > 0 or removidos > 0:
                                ...
                                with col_r1:
                                    st.success(f"➕ {novos} artigos adicionados") if novos > 0 else st.info("➕ 0 artigos adicionados")
                                with col_r2:
                                    st.warning(f"➖ {removidos} artigos removidos") if removidos > 0 else st.info("➖ 0 artigos removidos")
                            else:
    ...
    # ============================================================
    # SEÇÃO 2: ELIMINAR CONCORRENTE (COM PLACEHOLDER "Escolha Opção")
    # ============================================================
    with st.expander("❌ Eliminar Concorrente", expanded=False):
        # Lista de concorrentes ativos para eliminar
        lista_eliminar = sorted(concorrentes_ativos.keys())
        if not lista_eliminar:
            st.info("Nenhum concorrente ativo para eliminar.")
        else:
            # Adicionar opção vazia no início para funcionar como placeholder
            ...
            if not st.session_state.confirmar_eliminar:
                # Botão de eliminar só ativo se um concorrente válido estiver selecionado
                btn_eliminar_disabled = (concorrente_eliminar == "")
                if st.button(
                    "🗑️ Eliminar Concorrente",
                    type="primary",
                    key="btn_eliminar",
                    disabled=btn_eliminar_disabled
                ):
                    ...
                    if st.button("✅ Sim, eliminar", type="primary", key="btn_confirmar_eliminar"):
                        with st.spinner(f"A eliminar {concorrente_eliminar}..."):
                            if not os.access(EXCEL_PATH, os.W_OK):
                                st.error("❌ O ficheiro Excel está aberto noutro programa. Feche-o e tente novamente.")
                            else:
                                sucesso, registos, excel_removido, erro_msg = eliminar_concorrente(concorrente_eliminar)
                                if sucesso:
                                    # Remover do JSON de ativos
                                    if concorrente_eliminar in concorrentes_ativos:
                                        del concorrentes_ativos[concorrente_eliminar]
                                        guardar_concorrentes_ativos(concorrentes_ativos)
                                    st.success(f"✅ Concorrente '{formatar_nome_concorrente(concorrente_eliminar)}' eliminado!")
                                    if registos > 0:
                                        st.info(f"📊 {registos} registos removidos da BD.")
                                    if excel_removido:
                                        st.info("📁 Folha removida do Excel.")
                                    st.session_state.confirmar_eliminar = False
                                    st.cache_data.clear()
                                    time.sleep(2)
                                    st.rerun()
    ...
    # ============================================================
    # SEÇÃO 3: ADICIONAR NOVO CONCORRENTE (COM LIMPEZA DOS CAMPOS)
    # ============================================================
    with st.expander("➕ Adicionar Novo Concorrente", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
            # Se existir um valor guardado no estado, usa‑o; caso contrário, string vazia
            nome_default = st.session_state.get("temp_novo_concorrente", "")
            novo_concorrente = st.text_input(
                "Nome do concorrente (ex: PT_NovoConcorrente):",
                value=nome_default,
                placeholder="PT_Nome ou ES_Nome",
                key="input_novo_concorrente"
            )
        with col2:
            url_default = st.session_state.get("temp_nova_homepage", "")
            url_homepage = st.text_input(
                "🔗 URL da homepage:",
                value=url_default,
                placeholder="https://www.exemplo.com",
                key="input_nova_homepage"
            )

        st.caption("O nome deve começar com PT_ ou ES_. A URL será usada como link na lista de concorrentes.")

        if st.button("➕ Adicionar e Atualizar Catálogo", type="primary", key="btn_adicionar_concorrente"):
            ...
            elif not url_homepage:
                st.error("❌ A URL da homepage é obrigatória.")
            elif not url_homepage.startswith("http"):
                st.error("❌ A URL deve começar com http:// ou https://")
            elif novo_concorrente in concorrentes_ativos:
                st.error("❌ Esse concorrente já existe na lista de ativos.")
            else:
                with st.spinner(f"A criar '{novo_concorrente}' e a atualizar catálogo..."):
                    # 1. Adicionar ao JSON de ativos
                    concorrentes_ativos[novo_concorrente] = url_homepage
                    guardar_concorrentes_ativos(concorrentes_ativos)

                    # 2. Criar a folha base no Excel
                    sucesso, mensagem = adicionar_concorrente(novo_concorrente)
                    if not sucesso:
                        # Reverter JSON se falhar
                        del concorrentes_ativos[novo_concorrente]
                        guardar_concorrentes_ativos(concorrentes_ativos)
                        st.error(f"❌ {mensagem}")
                    else:
                        # 3. Executar script de atualização do catálogo
                        script_path = os.path.join(os.path.dirname(EXCEL_PATH), "gerar_template_excel.py")
                        if os.path.exists(script_path):
                            result = subprocess.run(
                                [sys.executable, script_path],
                            ...
                            if result.returncode == 0:
                                st.success(f"✅ Concorrente '{novo_concorrente}' adicionado e catálogo atualizado!")
                            else:
                                st.warning(f"⚠️ Concorrente adicionado, mas a atualização do catálogo falhou: {result.stderr}")
                        else:
                            st.warning("⚠️ Concorrente adicionado, mas o script de atualização não foi encontrado.")

                        # 4. Limpar os campos (definir valores vazios no estado)
                        st.session_state.temp_novo_concorrente = ""
                        st.session_state.temp_nova_homepage = ""
                        st.cache_data.clear()
                        time.sleep(2)
                        st.rerun()
    ...
    with st.expander("📋 Concorrentes Ativos", expanded=False):
        if concorrentes_ativos:
            conc_pt_lista = [c for c in concorrentes_ativos if c.startswith("PT_")]
            conc_es_lista = [c for c in concorrentes_ativos if c.startswith("ES_")]
            ...
            with col_pt:
                ...
                if conc_pt_lista:
                    for c in sorted(conc_pt_lista):
                        nome = formatar_nome_concorrente(c)
                        url = concorrentes_ativos.get(c)
                        if url:
            ...
            # Mostrar também folhas órfãs no Excel (se existirem)
            folhas_excel = set(concorrentes_excel)
            folhas_json = set(concorrentes_ativos.keys())
            orfas = folhas_excel - folhas_json
            if orfas:
                st.warning(f"⚠️ Existem {len(orfas)} folhas no Excel que não estão na lista de ativos: {', '.join(sorted(orfas))}")
                if st.button("🧹 Remover folhas órfãs do Excel"):
                    try:
                        wb = openpyxl.load_workbook(EXCEL_PATH)
                        for folha in orfas:
                            if folha in wb.sheetnames:
                                del wb[folha]
                        wb.save(EXCEL_PATH)
                        st.success("Folhas órfãs removidas.")
                        st.cache_data.clear()
                        time.sleep(1)
                        st.rerun()
                    except Exception as e:
                        st.error(f"Erro ao remover folhas: {e}")
        else:
            st.info("Nenhum concorrente ativo configurado.")

# ============================================================
# TAB 5 – GESTÃO DE LINKS E REFERÊNCIAS (COM BACKUP)
# ============================================================
with tab5:
    ...
    # Pasta de backups
    BACKUP_DIR = Path(EXCEL_PATH).parent / "backups"
    BACKUP_DIR.mkdir(exist_ok=True)

    def criar_backup():
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = BACKUP_DIR / f"links_concorrentes_backup_{timestamp}.xlsx"
        try:
    ...
    if df_dm_gestao.empty:
        ...
        st.error("⚠️ Nenhum concorrente encontrado no ficheiro Excel.")
        ...
        def obter_ultimo_preco(artigo: str, concorrente: str) -> tuple:
            """Retorna (preco_formatado, preco_valor, data_raw, referencia_bd)."""
            try:
                ...
                if result and result[0] and float(result[0]) > 0:
                    ...
                    data_raw = result[1] if result[1] else ""
                    referencia = result[2] if len(result) > 2 and result[2] else ""
        ...
        if 'workbook_cache' not in st.session_state:
            st.session_state.workbook_cache = {}

        def carregar_workbook(conc: str):
            if conc in st.session_state.workbook_cache:
                return st.session_state.workbook_cache[conc]
            try:
                wb = openpyxl.load_workbook(EXCEL_PATH)
                if conc not in wb.sheetnames:
                    return None, f"Folha '{conc}' não encontrada."
                ws = wb[conc]
                mapa = {}
                # Assumimos que a coluna D (índice 4) é a referência manual
                for row in ws.iter_rows(min_row=2, max_col=4):
                    if row[0].value:
                        artigo = str(row[0].value).strip()
                        url = row[2].value if len(row) > 2 else ""
                        ref_manual = row[3].value if len(row) > 3 else ""
                        mapa[artigo] = {
                            "linha": row[0].row,
                            "url": str(url) if url and str(url).lower() not in ("", "nan", "none") else "",
                            "ref_manual": str(ref_manual) if ref_manual and str(ref_manual).lower() not in ("", "nan", "none") else ""
                        }
                st.session_state.workbook_cache[conc] = (wb, mapa)
                return wb, mapa
            except Exception as e:
                return None, str(e)

        def salvar_e_recarregar(conc: str, wb):
            try:
                sucesso_backup, msg_backup = criar_backup()
                if not sucesso_backup:
                    st.warning(f"⚠️ Backup falhou: {msg_backup}")
                wb.save(EXCEL_PATH)
                if conc in st.session_state.workbook_cache:
                    del st.session_state.workbook_cache[conc]
                st.cache_data.clear()
                return True, "Guardado com sucesso."
            except PermissionError:
                return False, "Permissão negada. Feche o Excel."
            except Exception as e:
        ...
        with st.expander("🔎 Filtros", expanded=True):
            ...
            with col1:
                ...
                st.selectbox(
                    ...
                    index=idx_conc,
                    key="conc_widget",
                    on_change=on_concorrente_change
                )
            with col2:
        ...
        if not st.session_state.conc_sel_gestao:
            ...
            resultado = carregar_workbook(conc_atual)
            if isinstance(resultado, tuple) and resultado[0] is None:
                st.error(f"❌ {resultado[1]}")
                st.stop()
            wb, mapa_linhas = resultado
            ...
            if not arts_filtrados:
                ...
                artigo_sel = st.selectbox(
                    "📦 Artigo",
                    arts_filtrados,
                    format_func=lambda x: opcoes_formatadas.get(x, x),
                    index=idx_art,
                    key="art_widget",
                    on_change=on_artigo_change
                )

                info_artigo = mapa_linhas.get(artigo_sel, {})
                url_atual = info_artigo.get("url", "")
                ref_manual_atual = info_artigo.get("ref_manual", "")
                linha_artigo = info_artigo.get("linha")

                info_dm = dict_info_dm.get(artigo_sel, {})
                ultimo_preco, _, _, referencia_bd = obter_ultimo_preco(artigo_sel, conc_atual)
                ...
                with col_info:
                    ...
                    if referencia_bd:
                        st.markdown(f"**🔖 Ref. (BD):** {referencia_bd}")

                with col_edit:
                    input_key = f"url_input_{conc_atual}_{artigo_sel}_{st.session_state.refresh_key}"
                    novo_url = st.text_input("🔗 URL do produto", value=url_atual, placeholder="https://...", key=input_key)

                    # NOVO: campo para referência manual
                    ref_key = f"ref_input_{conc_atual}_{artigo_sel}_{st.session_state.refresh_key}"
                    nova_ref = st.text_input("🏷️ Referência (manual)", value=ref_manual_atual, placeholder="Ex: 16-144", key=ref_key)

                    col_btn1, col_btn2, col_btn3 = st.columns(3)
                    with col_btn1:
                        if st.button("💾 Guardar", use_container_width=True, key=f"save_{artigo_sel}"):
                            if not novo_url or not novo_url.startswith("http"):
                                st.error("❌ URL inválido (deve começar com http:// ou https://)")
                            else:
                                try:
                                    if linha_artigo is None:
                                        st.error("❌ Artigo não encontrado na folha.")
                                    else:
                                        ws = wb[conc_atual]
                                        ws.cell(row=linha_artigo, column=3, value=novo_url)
                                        ws.cell(row=linha_artigo, column=4, value=nova_ref)  # NOVO
                                        sucesso, msg = salvar_e_recarregar(conc_atual, wb)
                                        if sucesso:
                                    ...
                                    st.error(f"❌ Erro: {e}")
                    with col_btn2:
                        if st.button("🗑️ Limpar", use_container_width=True, key=f"clear_{artigo_sel}"):
                            try:
                                if linha_artigo is None:
                                    st.error("❌ Artigo não encontrado.")
                                else:
                                    ws = wb[conc_atual]
                                    ws.cell(row=linha_artigo, column=3, value="")
                                    ws.cell(row=linha_artigo, column=4, value="")  # NOVO
                                    sucesso, msg = salvar_e_recarregar(conc_atual, wb)
                                    if sucesso:
    ...
    with st.expander("📋 Artigos sem Link", expanded=False):
        if st.session_state.get("conc_sel_gestao"):
            if st.button("📋 Carregar artigos sem link", key="btn_carregar_sem_link"):
                with st.spinner("A carregar..."):
                    try:
                        conc = st.session_state.conc_sel_gestao
                        df_conc = pd.read_excel(EXCEL_PATH, sheet_name=conc, dtype=str)
                        ...
                        if col_url:
                            ...
                            for _, row in df_conc.iterrows():
                                ...
                                if not url or url.lower() in ("", "nan", "none"):
                                    artigos_sem_link.append({"Artigo": artigo})
                            if artigos_sem_link:
