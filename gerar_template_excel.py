...
Cria/actualiza o ficheiro Excel de links com:
  ...
  - Folhas de concorrentes: criadas se não existirem; preserva links e referências manuais já preenchidos
...
# ─────────────────────────────────────────────
#  FOLHAS DE CONCORRENTES (preserva dados, agora com coluna Referência)
# ─────────────────────────────────────────────

def actualizar_folha_concorrente(wb, nome, df_dm):
    cabecalhos = ["Artigo", "Descricao", "URL", "Referência"]
    larguras   = [14, 55, 70, 20]
    ...
    if nome not in wb.sheetnames:
        ...
        for r, (_, row) in enumerate(df_dm.iterrows(), 2):
            ...
            _celula(ws, r, 2, row["Descricao"], bg)
            _celula(ws, r, 3, "",               bg)
            _celula(ws, r, 4, "",               bg)   # NOVA COLUNA REFERÊNCIA
        ...
        for r_off, (_, row) in enumerate(novos.iterrows()):
            ...
            _celula(ws, r, 4, "",               COR_NOVO)   # NOVA COLUNA REFERÊNCIA
